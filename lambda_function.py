...
def get_welcome_response():
    ...
    speech_output = "Hey there," \
                    "Welcome to your custom application, Please note this application is still under development!"
...
############################################----------Sublist3r script-----------------###################################################
def get_url_response(intent):
    website= intent['slots']['nam']['value'] + ".com" #CHANGE ME (Replace your alexa skill relevent value instead of ['nam'])
    #!/usr/bin/env python
    # coding: utf-8
    # Sublist3r v1.0
    ...
    #########################################Email module started#################################
                ...
                MY_ADDRESS = 'user@email.com' #CHANGE ME
                PASSWORD = 'mysample-password' #CHANGE ME
                ...
                email="myuser@email.com" #CHANGE ME
                msg = MIMEMultipart()
                msg['From']=MY_ADDRESS
                msg['To']=email
    ...
    def interactive():
        args = parse_args()
        # domain = args.domain
        domain = intent['slots']['nam']['value'] + ".com"
    ...
    #adding https protocol and .com static domain
    website= intent['slots']['nam']['value'] + ".com"
    url="https://"+ intent['slots']['nam']['value'] + ".com"
    
    #checking whether given input URL exists or not
    req = Request(url)
    try:
        response = urlopen(req, timeout=3)
    except HTTPError as e:
        speech_output="The server can not fulfill the request"
        #print('Error code: ', e.code)
    except URLError as e:
        speech_output="Time Limit Exceed, We failed to reach a server."
        #print('Reason: ', e.reason)
    else:
        interactive()
        speech_output="Got the site" + website + "Please find results on your email"
        
        
    speechlet_response= speech_output
    reprompt_text = "Do you want me to do anything else"
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
    card_title, speech_output, reprompt_text, should_end_session))

############################################------Sublist3r script-ends-------------##########################################
...
def on_intent(intent_request, session):
    ...
    #------------------------------------------------------------------------------------------
    #ADD YOUR INTENT NAME HERE (CREATED FROM ALEXA DEVELOPER CONSOLE)
    if intent_name == "urlstatus": #CHANGE ME
        return get_url_response(intent)
    #-----------------------------------------------------------------------------------------    
